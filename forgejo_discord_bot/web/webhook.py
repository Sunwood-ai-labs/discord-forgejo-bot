...
from ..discord.notifications import send_issue_notification, send_comment_notification
...
def forgejo_webhook():
    ...
    try:
        ...
        # リポジトリ情報を取得
        repo_full_name = repository.get('full_name', '') if repository else ''
        print(f"repository: {repo_full_name}")
        
        if action in ['opened', 'closed', 'reopened']:
            print("issue通知分岐")
            asyncio.run_coroutine_threadsafe(
                send_issue_notification(bot, action, issue, repo_full_name),
                bot.loop
            )
            ...
            asyncio.run_coroutine_threadsafe(
                send_comment_notification(bot, issue, comment, repo_full_name),
                bot.loop
            )
        else:
